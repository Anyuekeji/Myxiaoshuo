...
@interface THMenuAction()
//@property (nonatomic) NSString      *title;
//@property (nonatomic) UIImage       *image;
@property (copy, nonatomic)void (^handler)(THMenuAction *);
@end
@implementation THMenuAction
+ (instancetype)actionWithTitle:(NSString *)title image:(UIImage *)image handler:(void (^)(THMenuAction *))handler{
    THMenuAction *action = [[THMenuAction alloc] initWithTitle:title image:image handler:handler];
    return action;
}
- (instancetype)initWithTitle:(NSString *)title image:(UIImage *)image handler:(void (^)(THMenuAction *))handler{
    if (self = [super init]) {
        _title = title;
        _image = image;
...
@property(nonatomic,copy) NSArray<THMenuAction *>   *actions;
...
+ (instancetype)menuWithActions:(NSArray<THMenuAction *> *)actions width:(CGFloat)width atPoint:(CGPoint)point{
    NSAssert(width>0.0f, @"width要大于0");
    THMenuView *menu = [[THMenuView alloc] initWithActions:actions width:width atPoint:point];
    return menu;
}
+ (instancetype)menuWithActions:(NSArray<THMenuAction *> *)actions width:(CGFloat)width relyonView:(id)view{
    NSAssert(width>0.0f, @"width要大于0");
    NSAssert([view isKindOfClass:[UIView class]]||[view isKindOfClass:[UIBarButtonItem class]], @"relyonView必须是UIView或UIBarButtonItem");
    THMenuView *menu = [[THMenuView alloc] initWithActions:actions width:width relyonView:view];
    return menu;
}

- (instancetype)initWithActions:(NSArray<THMenuAction *> *)actions width:(CGFloat)width atPoint:(CGPoint)point{
    if (self = [super init]) {
...
- (instancetype)initWithActions:(NSArray<THMenuAction *> *)actions width:(CGFloat)width relyonView:(id)view{
    if (self = [super init]) {
...
- (CGPoint)getRefPoint{
    ...
    CGPoint refPoint;
    if (absoluteRect.origin.y + absoluteRect.size.height + _actions.count * _menuCellHeight > k_ScreenHeight - 10) {
...
- (void)calculateArrowAndFrame{
    ...
    // 考虑向左右展示不全的情况，需要反向展示
    if (originX + width > k_ScreenWidth - 10) {
        originX = k_ScreenWidth - kDefaultMargin - width;
    }else if (originX < 10) {
    ...
    //如果不是根据关联视图，得算一次是否反向
    if (!_refView) {
        _isReverse = (originY + height > k_ScreenHeight - kDefaultMargin)?YES:NO;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    THMenuAction *action = _actions[indexPath.row];
